...
__device__ void unroll_last(volatile int* sdata, unsigned int tid){
...
__global__ void multiple_add(int *g_in_data, int *g_out_data, unsigned int n){
...
int main(){
    ...
    switch (blockSize) {
        case 512:
            multiple_add<512><<<num_blocks, 512, 512 * sizeof(int)>>>(dev_input_data, dev_output_data, n);
            break;
        case 256:
            multiple_add<256><<<num_blocks, 256, 256 * sizeof(int)>>>(dev_input_data, dev_output_data, n);
            break;
        case 128:
            multiple_add<128><<<num_blocks, 128, 128 * sizeof(int)>>>(dev_input_data, dev_output_data, n);
            break;
    }
